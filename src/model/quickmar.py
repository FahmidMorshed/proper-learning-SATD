...
class QUICKMAR(object):
    def __init__(self, dataset, enough, atleast, clf, step, enable_est=False, stopat=.95):
        ...
        self.body.loc[:, 'code'] = 'undetermined'
    ...
    def get_ensemble_ids(self):
        try:
            a = self.body.loc[self.body['no_vote'] <= self.ensemble_threshold]
            a = a.sample(self.step)
        except ValueError:
            return self.get_random_ids()
        return a.index
    ...
    def get_neg_help(self):
        try:
            a = self.body.loc[self.body['yes_vote'] < 3 & (self.body['code'] == 'undetermined')]
            prob = self.clf.predict_proba(self.csr_mat[a.index])[:, 1]
    ...
    def get_pos_help(self):
        try:
            a = self.body.loc[self.body['no_vote'] < 3 & (self.body['code'] == 'undetermined')]
            prob = self.clf.predict_proba(self.csr_mat[a.index])[:, 1]
            order = np.argsort(prob)[::-1][:self.step]
        except ValueError:
            print("Getting RANDOM")
    ...
    def train(self, pne=True, weighting=True):
        ...
        try:
            self.clf.partial_fit(self.csr_mat[sample_ids], labels[sample_ids])
        except ValueError:
            self.clf.fit(self.csr_mat[sample_ids], labels[sample_ids])

        ## aggressive undersampling ##
        if len(poses) >= self.enough:
            train_dist = self.clf.decision_function(self.csr_mat[all_neg_ids])
            ...
            train_dist = self.clf.decision_function(self.csr_mat[unlabeled_ids])
            pos_at = list(self.clf.classes_).index("yes")
            if pos_at:
                train_dist = -train_dist
        ...
        uncertain_id, uncertain_prob = self.uncertain(self.clf)
        certain_id, certain_prob = self.certain(self.clf)

        if self.enable_est:
            self.est_num, self.est = self.estimate_curve(self.clf, num_neg=len(sample) - len(poses))
    ...
    def estimate_curve(self, clf, num_neg=0):
        ...
        while (True):
            C = Counter(y[all])[1] / (num_neg*self.scale)
